...
local function determine_place_direction(surface, position, drill_prototype)
    ...
    -- If the mining drill is not square, this code can't cleanly handle it, so fallback to north.
    if offset_width ~= offset_height then
...
local function construct_sand_extractor(event)
    ...
    local direction = determine_place_direction(surface, position, drill_prototype)

    local is_ghost = (not cursor_stack_valid)
    or event.input_name == "build-ghost"
    or event.input_name == "super-forced-build"

    if surface.entity_prototype_collides(name, position, false) then return end

    -- prevents pressure dome placement, base game handles the floating text for this
    if surface.get_tile(position).hidden_tile then return end

    -- replaced the manual calculations with player.can_place_entity for build reach
    if not is_ghost and not player.can_place_entity {
        name = name,
        position = position,
        direction = direction,
    } then return end
...
-- "build", "build-ghost", and "super-forced-build" are custom keybind inputs.
